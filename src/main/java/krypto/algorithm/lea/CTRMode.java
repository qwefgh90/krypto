...
public class CTRMode extends LeaAlgorithm {

    // 객체 생성
    BlockCipherMode cipher = new LEA.CTR();
    byte[] counter;
    public CTRMode(String masterKey, int fixedKeySize, byte[] counter) {
        super(masterKey, fixedKeySize, Mode.CTR);
    ...
    public byte[] encrypt(byte[] bytes) {
        ...
        cipher.init(BlockCipher.Mode.ENCRYPT, keyBytes, counter);
    ...
    public byte[] decrypt(byte[] bytes) {
        cipher.init(BlockCipher.Mode.DECRYPT, keyBytes, counter);
