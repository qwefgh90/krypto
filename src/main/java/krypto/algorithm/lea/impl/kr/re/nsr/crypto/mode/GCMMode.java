...
import krypto.algorithm.lea.impl.kr.re.nsr.crypto.util.Pack;
...
import krypto.algorithm.lea.impl.kr.re.nsr.crypto.BlockCipherModeAE;
...
public class GCMMode extends BlockCipherModeAE {
	...
	public byte[] doFinal() {
		...
		Arrays.fill(block, (byte) 0x00);
		Pack.intToBigEndian(aadlen *= 8, block, 4);
		Pack.intToBigEndian(msglen *= 8, block, 12);
		ghash(macBlock, block, blocksize);

		engine.processBlock(initialCtr, 0, block, 0);
		Ops.XOR(macBlock, block);
	...
	private int decryptBlock(byte[] out, int outOff, int len) {
		...
		Ops.XOR(out, outOff, block, 0, inBuffer, 0, len);
	...
	private void init_8bit_table() {
		...
		for (int j = 2; j < 256; j <<= 1) {
			for (int k = 1; k < j; k++) {
				Ops.XOR(hTable[j + k], hTable[j], hTable[k]);
			}
		}
	}
	...
	private void ghash(byte r[], byte data[], int data_len) {
		...
		if (len > 0) {
			Ops.XOR(hashBlock, 0, data, pos, len);
			gfmul(hashBlock, hashBlock);
		}
	...
	private void gfmul(byte r[], byte x[]) {
		...
		Ops.XOR(r, mulBlock, hTable[rowIdx]);
	}
