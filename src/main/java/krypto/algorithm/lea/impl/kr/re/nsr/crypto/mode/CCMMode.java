...
import krypto.algorithm.lea.impl.kr.re.nsr.crypto.util.Hex;
...
public class CCMMode extends BlockCipherModeAE {
	...
	public byte[] doFinal() {
		...
		Hex.toBytes(msglen, block, noncelen + 1, 15 - noncelen);
	...
	private void processAAD() {
		...
		if (aad.length < 0xff00) {
			alen = 2;
			Hex.toBytes(aad.length, block, 0, 2);

		} else {
			alen = 6;
			block[0] = (byte) 0xff;
			block[1] = (byte) 0xfe;
			Hex.toBytes(aad.length, block, 2, 4);
		}
